...
计算图像上每个像素点的梯度大小和方向：
水平方向梯度: $\quad G_x(x, y)=I(x+1, y)-I(x-1, y)$ 垂
直方向梯度: $\quad G_y(x, y)=I(x, y+1)-I(x, y-1)$ 每个像 素点在 $(x, y)$ 处的梯度幅值和方向分别为:
幅值: $G(x, y)=\sqrt{G_x(x, y)^2+G_y(x, y)^2}$ 方向: $\alpha$ $=\arctan \frac{G_y(x, y)}{G_x(x, y)}$
